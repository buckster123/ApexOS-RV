...
- [ ] P3.1 `uart.rs`: NS16550A driver — poll LSR (offset 5) bit 5 (THR empty) before writing THR (offset 0); `impl core::fmt::Write`; QEMU needs no divisor init (leave a `// real hardware: init clocks/divisor` breadcrumb)
- [ ] P3.2 `print!` / `println!` macros over a globally reachable writer (single hart ⇒ the `riscv` crate's single-hart `critical-section` impl is sufficient locking; verify the feature name in the pinned `riscv` version)
- [ ] P3.3 `qemu.rs`: SiFive test device at `0x10_0000` — write `0x5555` = exit(0); `((code as u32) << 16) | 0x3333` = exit(code)
- [ ] P3.4 Panic handler: print `KERNEL PANIC: {info}` then `qemu::exit_fail(1)`
- [ ] P3.5 Temporary proof: a `#[cfg(feature = "panic-test")]` path that panics on purpose
...
- [ ] P4.1 Reserve the heap: an explicit `static` `[MaybeUninit<u8>; N]` region (simplest, version-proof; recommended) — riscv-rt 0.18's linker-provided heap symbols are the alternative if its docs make that cleaner
- [ ] P4.2 `heap.rs`: `embedded-alloc` as `#[global_allocator]`, init once at boot before first allocation
- [ ] P4.3 `extern crate alloc;` and a smoke test: build a `String` with `format!`, push to a `Vec`, print, drop
- [ ] P4.4 Decide + document OOM policy: default allocator behavior on failure is a panic path — our printing panic handler makes that visible; note heap size (1 MiB default) here
...
- [ ] P5.1 **Pristine vendor commit:** copy `apexos-protocol/` from upstream *at the pin* into `vendor/apexos-protocol/` byte-identical (its `{ workspace = true }` deps resolve against our P1.1 `[workspace.dependencies]`); add it to `[workspace] members`; copy upstream `LICENSE` into `vendor/apexos-protocol/`; write `vendor/apexos-protocol/UPSTREAM.md` (source repo, pin SHA, file list, "patches: none yet"). Gate: `cargo hosttest` runs the crate's own wire-lock tests green, untouched
- [ ] P5.2 **Patch commit — the `no_std` gate:** concretize deps (`serde`/`serde_json` with `default-features = false`; `serde/derive` kept); features `default = ["std"]`, `std = ["serde/std", "serde_json/std"]`, `alloc = ["serde/alloc", "serde_json/alloc"]`; `#![cfg_attr(not(feature = "std"), no_std)]`; `extern crate alloc` under `not(std)`; `std::fmt` → `core::fmt`; `String`/`Vec` from `alloc` under `not(std)`; `Map` alias for the **one** `HashMap` field (`RegisterMcpServer.env: HashMap<String, String>` — `String` keys are `Ord`, no HashMap-specific API in use, verified in review). Update `UPSTREAM.md` patch list
- [ ] P5.3 Host proofs: `cargo hostcheck` green (std default); `cargo check -p apexos-protocol --no-default-features --features alloc --target x86_64-unknown-linux-gnu` green; a wire-compat test asserting a `Map`-bearing proposal serializes to the same JSON object under both map types (semantic equality — JSON objects don't guarantee key order anyway)
- [ ] P5.4 Metal consumption: kernel deps gain `apexos-protocol = { path = "../vendor/apexos-protocol", default-features = false, features = ["alloc"] }` + `serde_json = { version = "1", default-features = false, features = ["alloc"] }`; in-kernel round-trip: construct, serialize, re-parse, assert equality, print — covering the **confirmed-risky variants**: a float-bearing `SensorReading::AirQuality`, `VastInstanceLaunched` (`f64`), and a `ToolCall` with a `serde_json::Value` args tree
- [ ] P5.5 Commit messages written for a reviewer (the P5.2 diff is the artifact ApexOS-RS would merge; note the wire-compat proof in its message)

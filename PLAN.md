...
- [x] P12.5 **Live colony runs completed 2026-07-19** — two transcripts in `docs/`: `live-run-2026-07-19.log` (session 28, first contact: APEX returned a genuine `GOAL_STEP: blocked` — *"no RISC-V substrate exists"* — over a socket served by that substrate; kernel parked the goal `Blocked` with APEX's words as detail, honest exit 2) and `live-run-2026-07-19-encore.log` (session 29: objective carried the context via the new `APEXRV_OBJECTIVE` knob; APEX acknowledged the substrate and finished `done` on step 1 — upstream's early-finish path — kernel exit 0, terminal marker printed). Both non-normative evidence; both parsed live by the same driver. Tagged `v2.0.0`
...
- 2026-07-19 — **v2.0.0** — the mesh arc complete, live-proven: session 28 (APEX blocked the goal, unaware it was answering *through* the substrate it declared missing — the Blocked path's first unscripted exercise) and session 29 (context-bearing objective → APEX verified the link and finished `done` step 1). The colony's LLM proposed, metal code disposed, over a real WebSocket, both ways. Ops footgun recorded: all feature builds share one ELF path — rebuild for the intended feature set before running a gate script, or you boot the previous variant (a mesh-goal ELF with no mock listening panics on the 10 s TCP-send timeout, as designed).

...
3. **`unsafe` stays in the HAL.** Only `uart.rs`, `qemu.rs`, `heap.rs`, and boot/trap glue may contain `unsafe`, and every block carries a `// SAFETY:` comment naming the invariant. `agent-core` is 100% safe, `no_std`, and host-testable.
